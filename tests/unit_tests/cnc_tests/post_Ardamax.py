...
csvLogFile = 'C:\\Users\\Public\\ReWare Agent\\logs\\1_POST.csv'
...
try:
    path = r'c:\temp_deletemeplease'.replace('\\', os.sep)
    orig_path = path
    if '%USERNAME%' in path:
        path = path.split('%USERNAME%')[0] + getpass.getuser() + path.split('%USERNAME%')[1]
    if 'system32' in path:
        try:
            stats = os.stat(path.split('system32')[0] + 'SYSNATIVE' + path.split('system32')[1])
    ...
    row = [str(IPAddr), gettime(), "File", r"c:\temp_deletemeplease", "Create", "Success"]
    logcsv(row)
except Exception as e:
    row = [str(IPAddr), gettime(), "File", r"c:\temp_deletemeplease", "Create", "Failed", str(e)]
    logcsv(row)
